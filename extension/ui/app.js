...
function time(value){return value?new Date(value).toLocaleString():'Not checked';}
...
async function load(){
  ...
  if(next.page?.token!==lastToken){manualResults.clear();lastToken=next.page?.token;}
  state=next;render();
}
...
function renderOverview(){
  ...
  if(!page){out.append(empty('Know what is running.','Inspect the current page to see technologies, version evidence, and matching advisories.'));out.append(node('p','Only package names and versions leave the browser for OSV checks. Website URLs and page contents stay local.','small muted'));return;}
...
function observation(){const p=state.page;return {tool:'TRFFN VulnWatch',version:'0.1.0',notice:'Passive observations and affected-version matches. Exploitability unverified. Top-level document only.',origin:p?.origin,observedAt:p?.finished?new Date(p.finished).toISOString():null,components:p?.components||[],coverage:{headers:p?.headersAvailable,runtime:p?.runtimeAvailable,resourceLimit:p?.limited}};}
...
$('clear-data').addEventListener('click',async()=>{try{await send('clear');manualResults.clear();await load();feedback('Observations, OSV cache, and mutes cleared.');}catch(e){feedback(e.message,true);}});
